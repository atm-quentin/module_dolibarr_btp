...
/**
 *	\file       htdocs/core/modules/propale/doc/pdf_chargement_camion.modules.php
 *	\ingroup    propale
 ...
 */
class pdf_chargement_camion extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
		...
		if ($conf->propal->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				{ 
					...
					$pdf->SetXY($this->posxloaded, $curY);
					// Enough for 6 chars
					if($conf->global->PRODUCT_USE_UNITS)
					{
						$pdf->MultiCell($this->posxqty-$this->posxloaded-0.8, 4, "", 0, 'R');
					}
					// Quantity
...
//		return ($tab2_top + ($tab2_hl * $index));
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		if (empty($hidetop))
		{
			$pdf->line($this->posxloaded-1, $tab_top, $this->posxloaded-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxloaded-1, $tab_top+1);
			
			if($conf->global->PRODUCT_USE_UNITS)
			{
				$pdf->MultiCell($this->posxqty-$this->posxloaded-1,2, $outputlangs->transnoentities("Chargé"),'','C');
			}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("Chargement de Camion");
